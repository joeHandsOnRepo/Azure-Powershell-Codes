# Please go to line 69 and replace the list with the correct tag keys in your VMs
param(
...
foreach ($vm in $WindowsRunningVMs) {
    ...
    #============================================================================================================
    # get all the tags for the current vm
    $tagKeyList = @("tag1","tag2", "tag3", "tag4", "tag5", "tagN")
    $vmTags = $currentVM.tags
    foreach ($tagKey in $tagKeyList) {
        if ($vmTags.Keys -contains $tagKey) {
            $currentVmTagValue = $vmTags[$tagKey]
        }
        else {
            $currentVmTagValue = "None"
        }
        $CommonVmsInfo_object | Add-Member -MemberType NoteProperty -Name "Tag $tagKey" -Value $currentVmTagValue
    }
    #============================================================================================================
